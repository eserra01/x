...
class StockPushAndPullWizard(models.TransientModel):
  ...
  def print_xls_report(self):
    ...
    ### BUSCAMOS LOS PRODUCTOS PERTENECIENTES A ESA CATEGORIA
    prod_ids = self.env['product.template'].search([('categ_id','=',categ_id.id)]).ids

    if not prod_ids:
      raise ValidationError("La categoría no cuenta con productos para procesar.")
    else:
      product_ids = product_obj.browse(prod_ids)
    ...
    for product_id in product_ids:
      ...
      data.update({
        product_id.id : records
      })
    if not data:
      raise ValidationError("No se encontró información para procesar")

    return self.env.ref('pabs_reports.stock_push_and_pull_report_xlsx').report_action(self, data={'products' : data})
...
class StockPushAndPullReportXLSX(models.AbstractModel):
  ...
  def generate_xlsx_report(self, workbook, data, lines):
    ...
    ### ENCABEZADOS
    count = 0
    for rec_id in data['products'].keys():
      product_id = product_obj.browse(int(rec_id))
      sheet.write(count,1,'Articulo Seleccionado:', bold_format)
      sheet.write(count,2,product_id.name,bold_format)
      count += 1
      for row, row_data in enumerate(HEADERS):
        sheet.write(count, row, row_data, bold_format)
      count+=1

      for row_data in data['products'][rec_id]:
