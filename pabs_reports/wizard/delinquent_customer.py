      ...
      SELECT *
        FROM (
            ...
            CASE
                ...
                WHEN c.contract_status_item = 21 AND TO_CHAR( (current_date AT TIME ZONE 'Mexico/General')  :: DATE,  'yyyy-mm-dd') - GREATEST(c.date_first_payment, (Select MAX(date_receipt) from account_payment as last where last.contract = c.id)) <= (
                        CASE
        ...
        WHERE
            am.type = 'out_invoice'
                AND c.invoice_date <= TO_CHAR( (current_date AT TIME ZONE 'Mexico/General')  :: DATE,  'yyyy-mm-dd')
