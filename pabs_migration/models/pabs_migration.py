...
class PabsMigration(models.Model):
  ...
  def crear_contratos(self, company_id, limite = 0):   
    ...
    for index, d in enumerate(rows, 1):          
      ...
      sale_employee_id = employee_obj.search([('barcode','=',d.get('cod_asistente'))])       
      ...
      price_list_item_id = self.env['product.pricelist.item'].search([('product_id','=',product_id.id)], limit=1)
      ...
      municipality_id = locality_obj.search([('name','=',str(d.get('municipio')).strip().upper())])  
      if not municipality_id:
        if company_id == COMPANY_SAL:
          municipality_id = locality_obj.search([('name','=','SALTILLO')])
        elif company_id == COMPANY_MON:
          municipality_id = locality_obj.search([('name','=','MONCLOVA')])
      ...
      # Se busca la colonia
      neighborhood_id = self.env['colonias'].search([('name','=',str(d.get('colonia')).strip().upper())],limit=1) 
      # Se busca la colonia de cobro
      toll_neighborhood_id = self.env['colonias'].search([('name','=',str(d.get('colonia_cobro')).strip().upper())],limit=1)

      # Se crea el cobrador si no existe
      debt_collector_id = employee_obj.search([('barcode','=',d.get('cod_cobrador'))])       
